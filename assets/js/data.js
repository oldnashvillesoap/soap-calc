...
data.oils = [
    ...
    { name:  "Carnauba (Copernicia cerifera) wax" , ins: 84 , iodine: 11 , lauric: 0 , linoleic: 0 , linolenic: 0 , myristic: 0 , oleic: 0 , palmitic:  0 , ricinoleic:  0 , stearic: 0 , sap: 0.087 , eicosenoic: 0 , docosenoid: 0 , docosadienoic: 0 , erucic: 0 , caprylic:  0 , capric: 0 },
...
data.fn.propertyValueForOil = function( oil, fats ) {
    return _.reduce( fats, function( total, fat ) {
        return total + oil[fat];
    }, 0 );
};

data.fn.properties = function( oil ) {
    return {
        hardness: data.fn.propertyValueForOil( oil, ['lauric', 'myristic', 'palmitic', 'stearic', 'caprylic', 'capric'] ),
        cleansing: data.fn.propertyValueForOil( oil, ['lauric', 'myristic', 'caprylic', 'capric'] ),
        bubbly: data.fn.propertyValueForOil( oil, ['lauric', 'myristic', 'ricinoleic', 'caprylic', 'capric'] ),
        stable: data.fn.propertyValueForOil( oil, ['palmitic', 'stearic', 'ricinoleic'] ),
        longevity: data.fn.propertyValueForOil( oil, ["palmitic", "stearic"] ),
        condition: data.fn.propertyValueForOil( oil, ['ricinoleic', 'oleic', 'linoleic', 'linolenic', 'eicosenoic', 'docosenoid', 'docosadienoic', 'erucic'] )
    }
};

data.fn.totalSaponifiable = function( oil ) {
    return data.fn.propertyValueForOil( oil, _.keys( data.sapyOils ) );
};

data.fn.saturationBreakdowns = function() {
    return {
        saturated: _.keys(_.pickBy(data.sapyOils, (val) => _.endsWith(val, ':0'))),
        monoUnSaturated: _.keys(_.pickBy(data.sapyOils, (val) => _.endsWith(val, ':1'))),
        polyUnSaturated: _.keys(_.pickBy(data.sapyOils, (val) => _.endsWith(val, ':2') || _.endsWith(val, ':3') ))
    };
};

data.fn.fatBreakdown = function( oil ) {
    var fats = _.keys( data.sapyOils );
...
data.fn.saturationBreakdown = function( oil ) {
    return _.transform( data.fn.saturationBreakdowns(), function( result, fats, group ) {
        result[ group ] = data.fn.propertyValueForOil( oil, fats );
    } );
};

data.fn.buildOil = function( oil, id ) {
    return {
        id: id,
        name: oil.name,
        iodine: oil.iodine,
        ins: oil.ins,
        sap: oil.sap,
        breakdown: data.fn.fatBreakdown( oil ),
        properties: data.fn.properties( oil ),
        saturation: data.fn.saturationBreakdown( oil ),
        totalSaponifiable: data.fn.totalSaponifiable( oil )
    }
};

_.each( data.oils, function( oil, index ) {
    data.build.push(data.fn.buildOil( oil, index + 1 ));
} );